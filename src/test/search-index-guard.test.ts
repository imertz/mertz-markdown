...
describe('search index cannot silently diverge from the database', () => {
  ...
  it('invalidates search when vault sync replaces local storage', () => {
    const hook = readFileSync(join(SRC, 'hooks', 'useDocuments.ts'), 'utf8')
    const start = hook.indexOf('const refreshFromStorage')
    const end = hook.indexOf('\n\n  return {', start)
    const refresh = hook.slice(start, end)

    expect(start).toBeGreaterThan(-1)
    expect(refresh).toContain('invalidateIndex()')
    expect(refresh).toContain('setContentRevision')
  })
})
