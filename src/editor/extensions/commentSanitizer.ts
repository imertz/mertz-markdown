    ...
    return [
      new Plugin({
        ...
        appendTransaction(transactions, _oldState, newState) {
          ...
          // `null` means threads have not loaded yet. Stripping now would
          // wipe every anchor in a document that simply hasn't finished opening
          // — including one being reloaded by a vault sync, whose incoming
          // anchors are legitimate but whose thread records land a tick later.
