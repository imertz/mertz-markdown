...
let generation = 0
let building: { generation: number; promise: Promise<Index> } | null = null

/** Which record ids belong to a document, so removal never needs a search. */
let idsByDoc = new Map<string, string[]>()
...
async function addPassages(
  ...
  for (const passage of passages) {
    const bucket = registry.get(passage.docId)
    if (bucket) bucket.push(passage.id)
    else registry.set(passage.docId, [passage.id])
  }
}

async function build(buildGeneration: number): Promise<Index> {
  ...
  for (const record of [...live, ...trashed]) {
    await addPassages(fresh, freshIds, [
      ...collectPassages(record),
      ...collectCommentPassages(record, commentsByDoc.get(record.id) ?? []),
    ])
  }

  // A vault pull can invalidate the index while this asynchronous build is in
  // flight. Only the newest generation is allowed to become authoritative.
...
export function ensureIndex(): Promise<Index> {
  ...
  if (building?.generation === current) return building.promise

  const promise = build(current)
    .then(target => (current === generation ? target : ensureIndex()))
    .finally(() => {
      if (building?.generation === current) building = null
    })
  building = { generation: current, promise }
  return promise
}

/**
 * Forget the derived index after storage has been replaced externally.
 *
 * Incrementing the generation also disqualifies an initial build already in
 * flight. Its caller transparently joins the next build instead of publishing
 * a snapshot taken before the vault pull completed.
 */
...
async function replaceDocument(
  ...
  await addPassages(target, registry, passages)
}
...
export async function reindexDocument(record: DocumentRecord): Promise<void> {
  const target = index
  const registry = idsByDoc
  if (!target) return
  const comments = await listCommentsForDoc(record.id)
  // A remote refresh may have invalidated this target while the comments were
  // loading. The next lazy build will include the write from IndexedDB.
  if (target !== index) return
  await replaceDocument(target, registry, record, [
    ...collectPassages(record),
...
export async function dropDocument(docId: string): Promise<void> {
  ...
  const registry = idsByDoc
  if (!target) return
  const previous = registry.get(docId)
  if (previous?.length) await removeMultiple(target, previous)
