...
export function useThreads(docId: string | null, reloadToken = 0): ThreadsApi {
  ...
  const getKnownIds = useCallback((): ReadonlySet<string> | null => {
    // While a reload is in flight, `threads` still holds the *previous* read.
    // A vault sync advances the document and the reload token together, and
    // useMarkdownEditor's setContent runs synchronously while the thread read
    // is still a pending promise. Reporting the stale set as authoritative
    // would make CommentSanitizer judge every anchor arriving with the new
    // content foreign and strip it on the spot — and the next ordinary edit
    // would autosave that loss. `null` is the explicit "not loaded yet"
    // signal; an authoritative empty set must still remove foreign anchors.
