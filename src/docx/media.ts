 ...
 * and this app stores almost every local image as WebP, because both
 * `IMPORTED_IMAGE_MIME` and `CROPPED_IMAGE_MIME` are `image/webp`. Embedding
 * the stored bytes unchanged would therefore give most users a document full
 * of empty frames, so those two formats are re-encoded to PNG on the way out.
 */
